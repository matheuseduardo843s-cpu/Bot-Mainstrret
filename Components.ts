...
export class Component<C extends CacheType = CacheType> {
    ...
    constructor(data: ComponentData<C>){
        if (typeof data.customId === "string"){
            ...
            log.success(ck.green(`${ck.cyan.underline(data.customId)} componentes registrados com sucesso💫!`));
        } else {
            Component.logical.push(data as any);
            const name = (data as { name: string }).name;
            log.success(ck.green(`${ck.cyan.underline(name)} componentes registrados com sucesso💫!`));
        }
    }
}